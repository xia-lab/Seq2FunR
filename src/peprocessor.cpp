...
bool PairEndProcessor::processPairEnd(ReadPairPack* pack, ThreadConfig* config, TransSearcher * transSearcher) {
			...
			if (r1 != NULL && result1 == PASS_FILTER && r2 != NULL && result2 == PASS_FILTER) {
				...
				if (mOptions->outputToSTDOUT && !mOptions->merge.enabled) {
					...
					if (ov.overlapped) {
						...
						if (result == PASS_FILTER) {
							transSearcher->transSearch(merged, tmpReadsTaxonKOPair);
						} else {
							transSearcher->transSearch(r1, r2, tmpReadsTaxonKOPair);
						}
						delete merged;
					} else {
						transSearcher->transSearch(r1, r2, tmpReadsTaxonKOPair);
				...
				if (!tmpReadsTaxonKOPair.first.empty()) {
					...
					if (mLeftWriter && mRightWriter) {
						outstr1 += r1->toStringWithTag(tmpReadsTaxonKOPair.second.getToSpecies());
						outstr2 += r2->toStringWithTag(tmpReadsTaxonKOPair.second.getToSpecies());
					}
				}
	...
	mReadsMtx.lock();
	mOptions->transSearch.transReadsTaxonKOPairDeque.insert(mOptions->transSearch.transReadsTaxonKOPairDeque.end(),
