...
extern "C"
{
     ...
     */
    NCBI_XBLAST_EXPORT int Blast_GetJointProbsForMatrix(double ** probs,
            double                                                row_sums[],
            double                                                col_sums[],
            const char *                                          matrixName);

    /** Return true if frequency data is available for the given matrix name. */
    NCBI_XBLAST_EXPORT const double * Blast_GetMatrixBackgroundFreq(const char * matrix_name);

    /**
     *  Retrieve the background letter probabilities implicitly used in
     * constructing the score matrix matrix_name.
     */
    NCBI_XBLAST_EXPORT int Blast_FrequencyDataIsAvailable(const char * matrix_name);
