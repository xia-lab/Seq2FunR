...
static char const rcsid[] = "$Id: hspfilter_collector.c 443210 2014-08-12 13:55:24Z fongah2 $";
                                  ...
                                  BlastHSPList * hsp_list)
{
    ...
    if (results -> num_queries > 1)
    {
        ...
        for (index = 0; index < hsp_list -> hspcnt; index++)
        {
            ...
            {
                hsp_list_array[query_index]                = tmp_hsp_list = Blast_HSPListNew(params -> hsp_num_max);
        ...
        for (index = 0; index < results -> num_queries; index++)
        {
            if (hsp_list_array[index])
            {
                ...
                {
                    results -> hitlist_array[index] = Blast_HitListNew(params -> prelim_hitlist_size);
                }

                Blast_HitListUpdate(results -> hitlist_array[index], hsp_list_array[index]);
            }
        }
    ...
    else if (hsp_list -> hspcnt > 0)
    {
        ...
        {
            results -> hitlist_array[0] = Blast_HitListNew(params -> prelim_hitlist_size);
        }
        ...
        BlastHSPList *                       hsplist_in)
{
    ...
    if (!hit_list)
    {
        results -> hitlist_array[hsplist_in -> query_index]            = hit_list = Blast_HitListNew(
            params -> prelim_hitlist_size);
    }
     ...
     *  and then score, etc.
     */
    qsort(hsplist_in -> hsp_array, hsplist_in -> hspcnt, sizeof(BlastHSP *), s_ScoreCompareHSPWithContext);
    ...
    for (index = 0; index < hsplist_in -> hspcnt; index = next_index)
    {
         ...
         */
        for (next_index = index; next_index < hsplist_in -> hspcnt; ++next_index)
        {
        ...
        BlastQueryInfo *                              query_info)
{
    ...
    writer -> RunFnPtr   = (Blast_ProgramIsRpsBlast(col_param -> program))
                           ? &s_BlastHSPCollectorRun_RPS : &s_BlastHSPCollectorRun;
        ...
        Boolean                                                                    gapped_calculation)
{
    ...
    {
        prelim_hitlist_size = MIN(2 * prelim_hitlist_size, prelim_hitlist_size + 50);
    }
...
{
    BlastHSPWriterInfo * writer_info = malloc(sizeof(BlastHSPWriterInfo));
