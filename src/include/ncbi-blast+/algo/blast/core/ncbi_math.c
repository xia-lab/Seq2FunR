...
static char const rcsid[] = "$Id: ncbi_math.c 351814 2012-02-01 17:14:10Z ucko $";
                              ...
                              double * u)
{
    ...
    switch (order)
    {
        ...
        case 4 :
            value = y[4] - 4. * y[3] * y[1] - 3. * y[2] * y[2] + 12. * y[2] * (tmp = y[1] * y[1]);
            value -= 6. * tmp * tmp;
...
{
    4.694580336184385e+04, -1.560605207784446e+05, 2.065049568014106e+05, -1.388934775095388e+05, 5.031796415085709e+04,
    -9.601592329182778e+03, 8.785855930895250e+02, -3.155153906098611e+01, 2.908143421162229e-01,
    -2.319827630494973e-04, 1.251639670050933e-10
};
                               ...
                               Int4   order)
{
    ...
    switch (order)
    {
        ...
        default :
            tmp = BLAST_Factorial(order - 2) * BLAST_Powi(tmp, 1 - order) * (1. + (order - 1) * xgamma_dim / tmp);
                          ...
                          Int4   order)
{
    ...
    if (x < 0.)
    {
        ...
        if (order == 0)
        {
            ...
            if (((x < -0.1) && ((ceil(x) == x) || (sx < 2. * DBL_EPSILON))) || (sx == 0.))
            {
                return HUGE_VAL;
            }
...
static const double kPrecomputedFactorial[] =
{
    ...
    8222838654177922817725562880000000., 263130836933693530167218012160000000., 8683317618811886495518194401280000000.,
