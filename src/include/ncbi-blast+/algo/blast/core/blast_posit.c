...
static char const rcsid[] = "$Id: blast_posit.c 99676 2007-03-05 20:41:55Z kazimird $";
...
Kappa_posSearchItems * Kappa_posSearchItemsNew(unsigned int queryLength,
        ...
        double **                                           posFreqs)
{
    ...
    retval -> posMatrix = (int **) _PSIAllocateMatrix(queryLength, BLASTAA_SIZE, sizeof(int));
...
Kappa_posSearchItems * Kappa_posSearchItemsFree(Kappa_posSearchItems * posSearch)
{
    ...
    {
        posSearch -> posMatrix = (int **) _PSIDeallocateMatrix((void **) posSearch -> posMatrix,
                posSearch -> queryLength);
    }

    if (posSearch -> stdFreqRatios)
    {
        posSearch -> stdFreqRatios = _PSIMatrixFrequencyRatiosFree(posSearch -> stdFreqRatios);
    }
...
Kappa_compactSearchItems * Kappa_compactSearchItemsNew(const Uint1 * query,
        ...
        BlastScoreBlk *                                              sbp)
{
    ...
    retval = (Kappa_compactSearchItems *) calloc(1, sizeof(Kappa_compactSearchItems));
 ...
 * (blast_stat.c)
 */
static Blast_ScoreFreq * fillSfp(int **            matrix,
                                 int               matrixLength,
 ...
 * updated [in|out]
 */
static Boolean impalaScaleMatrix(Kappa_compactSearchItems * compactSearch,
                                 int **                     posMatrix,
                                 int **                     posPrivateMatrix,
                                 ...
                                 BlastScoreBlk *            sbp)
{
    ...
    int **            private_matrix;           /*
                                                 *  pointer to locally manipulated version of
                                                 *     the matrix
                                                 */
    int **            matrix;
    double            scalefactor;              /* local version of amount of scaling */
    double            divFactor;                /* 1/scalefacor */
    Blast_ScoreFreq * this_sfp, *return_sfp;    /*
                                                 *  score frequency pointers
                                                 *   to compute lambda
                                                 */
        ...
        while (done != TRUE)
        {
            for (c = 0; c < dim1; c++)
            {
                for (a = 0; a < dim2; a++)
                {
                    ...
                    {
                        matrix[c][a] = (int) ((factor * private_matrix[c][a]) / divFactor);
                    }
                }
            }

            this_sfp = fillSfp(matrix, dim1, compactSearch -> standardProb, scoreArray, return_sfp);
            ...
            new_lambda = Blast_KarlinLambdaNR(this_sfp, compactSearch -> kbp_psi[0] -> Lambda / scalingFactor);
    ...
    for (c = 0; c < dim1; c++)
    {
        for (a = 0; a < dim2; a++)
        {
            ...
            {
                matrix[c][a] = BLAST_Nint((double) private_matrix[c][a] * factor / kPSIScaleFactor);
            }
        }
    }
    ...
    for (c = 0; c < dim1; c++)
    {
        for (a = 0; a < dim2; a++)
        {
            if (BLAST_SCORE_MIN != private_matrix[c][a])
            {
                private_matrix[c][a] = BLAST_Nint((double) private_matrix[c][a] * factor * scalefactor);
            }
        }
    }
                        ...
                        BlastScoreBlk *            sbp)
{
    ...
    return ((impalaScaleMatrix(compactSearch, posSearch -> posMatrix, posSearch -> posPrivateMatrix, scalingFactor,
                               doBinarySearch, sbp) == TRUE) ? 0 : 1);
}
