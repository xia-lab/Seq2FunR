...
static char const rcsid[] = "$Id: blast_query_info.c 419225 2013-11-22 17:08:29Z camacho $";
                                   ...
                                   int               num_queries)
{
    ...
    retval -> contexts      = (BlastContextInfo *) calloc(retval -> last_context + 1, sizeof(BlastContextInfo));
    ...
    else
    {
        ...
        {
            retval -> contexts[i].query_index = Blast_GetQueryIndexFromContext(i, program);
...
BlastQueryInfo * BlastQueryInfoFree(BlastQueryInfo * query_info)
{
    if (query_info)
    {
        ...
        query_info -> pattern_info = SPHIQueryInfoFree(query_info -> pattern_info);
...
BlastQueryInfo * BlastQueryInfoDup(const BlastQueryInfo * query_info)
{
    ...
    retval -> contexts = BlastMemDup(query_info -> contexts, num_contexts * sizeof(BlastContextInfo));

    if (query_info -> pattern_info)
    {
        retval -> pattern_info = SPHIQueryInfoCopy(query_info -> pattern_info);
    }
                                 ...
                                 EBlastProgramType prog)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        info -> contexts[i].frame       = BLAST_ContextToFrame(prog, i);
        info -> contexts[i].query_index = Blast_GetQueryIndexFromContext(i, prog);
    }
}
                              ...
                              Int4                   query_index)
{
    ...
    if (!one_query_info_ptr ||!one_query_ptr ||!query_info ||!query || (query_index >= query_info -> num_queries))
    {
        return -1;
    }
    ...
    memcpy(one_query_info -> contexts, &query_info -> contexts[first_context], num_frames * sizeof(BlastContextInfo));
    ...
    one_query -> length   = one_query_info -> contexts[num_frames - 1].query_offset
