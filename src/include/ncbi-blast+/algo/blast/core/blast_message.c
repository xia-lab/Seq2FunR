...
static char const rcsid[] = "$Id: blast_message.c 442874 2014-08-07 19:26:31Z camacho $";
...
const char * kBlastErrMsg_CantCalculateUngappedKAParams =
    ...
    "to an invalid query sequence or its translation. Please verify the " "query sequence(s) and/or filtering options";
                    ...
                    int              context)
{
    ...
    switch (error_code)
    {
        case BLASTERR_IDEALSTATPARAMCALC :
            new_msg -> message  = strdup("Failed to calculate ideal Karlin-Altschul " "parameters");
        ...
        case BLASTERR_REDOALIGNMENTCORE_NOTSUPPORTED :
            new_msg -> message = strdup("Composition based statistics or " "Smith-Waterman not supported for your "
                                        "program type");
            new_msg -> severity = eBlastSevError;
            new_msg -> context  = context;
        ...
        case BLASTERR_SEQSRC :
            new_msg -> message  = strdup("search cannot proceed due to errors " "retrieving sequences from databases");
            new_msg -> severity = eBlastSevFatal;
            new_msg -> context  = context;
    ...
    {
        new_msg -> origin = SMessageOriginNew(file_name, (unsigned int) lineno);
    }
