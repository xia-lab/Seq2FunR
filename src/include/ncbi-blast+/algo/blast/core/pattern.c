...
static char const rcsid[] = "$Id: pattern.c 134303 2008-07-17 17:42:49Z camacho $";
                            ...
                            const SPHIPatternSearchBlk * pattern_blk)
{
    ...
    Int4                       prefixMatchedBitPattern = 0;    /*
                                                                * indicates where pattern aligns
    ...
    Int4                       mask;                           /*
                                                                * mask of input pattern positions after which
    ...
    for (i = 0; i < len1; i++)
    {
         ...
         * first position
         */
        prefixMatchedBitPattern = ((prefixMatchedBitPattern << 1) | maskShiftPlus1)
                               ...
                               const SPHIPatternSearchBlk * pattern_blk)
{
    ...
    {
        pos                     = 4 - pos;
        prefixMatchedBitPattern = ((kMatchMask * ((1 << (pos + 1)) - 1) * 2) + (1 << (pos + 1)) - 1)
    ...
    for (i = 0; i < end; i++)
    {
        if ((tmp = (prefixMatchedBitPattern & pattern_items -> dna_items -> DNAwhichPrefixPosPtr[seq[i]])))
        {
            for (j = 0; j < 4; j++)
            {
                if (tmp & kMatchMask)
                {
                    ...
                    hitArray[twiceNumHits++] = i * 4 + j + pos - s_LenOf(tmp & kMatchMask, kMatchMask) + 1;
                }
        ...
        prefixMatchedBitPattern = (((prefixMatchedBitPattern << 4) | kMask2)
                           ...
                           const SPHIPatternSearchBlk * pattern_blk)
{
    ...
    Int4 *              prefixMatchedBitPattern;    /*
                                                     * see similar variable in
                                                     * _PHIBlastFindHitsShort
                                                     */
    Int4                twiceNumHits = 0;           /* counter for hitArray */
    Int4 *              mask;                       /*
                                                     * local copy of match_maskL version of pattern
                                                     * indicates after which positions a match can be declared
                                                     */
    Int4 *              matchResult;                /*
                                                     * Array of words to hold the result of the
    ...
    for (i = 0; i < len1; i++)
    {
        ...
        if (_PHIPatternWordsBitwiseAnd(matchResult, prefixMatchedBitPattern, pattern_items -> match_maskL, num_words))
        {
            hitArray[twiceNumHits++] = i;
            hitArray[twiceNumHits++] = i - s_LenOfL(matchResult, pattern_items -> match_maskL, num_words) + 1;
        }
    }
                               ...
                               const SPHIPatternSearchBlk * pattern_blk)
{
    ...
    Int4                     twiceHitsOneCall;          /*
                                                         * twice the number of hits in one call to
                                                         *     _PHIBlastFindHitsShort
                                                         */
    ...
    if (is_dna)
    {
        word_items -> dna_items ->
             DNAwhichPrefixPosPtr = multiword_items -> dna_items -> DNAprefixSLL[most_specific_word];
        word_items -> dna_items ->
             DNAwhichSuffixPosPtr = multiword_items -> dna_items -> DNAsuffixSLL[most_specific_word];
    }

    /* find matches to most specific word of pattern */
    twiceNumHits = s_FindHitsShortHead(hitArray, seq, 0, len, is_dna, pattern_blk);
    ...
    for (wordIndex = most_specific_word + 1; wordIndex < multiword_items -> numWords; wordIndex++)
    {
        ...
        if (is_dna)
        {
            word_items -> dna_items -> DNAwhichPrefixPosPtr = multiword_items -> dna_items -> DNAprefixSLL[wordIndex];
            word_items -> dna_items -> DNAwhichSuffixPosPtr = multiword_items -> dna_items -> DNAsuffixSLL[wordIndex];
        }
        ...
        {
            twiceHitsOneCall = s_FindHitsShortHead(&hitArray1[nextPosInHitArray],
                    seq,
                    hitArray[hitIndex2] + 1,
                    MIN(len - hitArray[hitIndex2] - 1,
                        extra_items -> spacing[wordIndex - 1] + extra_items -> numPlacesInWord[wordIndex]),
                    is_dna,
                    pattern_blk);
    ...
    for (wordIndex = most_specific_word - 1; wordIndex >= 0; wordIndex--)
    {
        word_items -> whichPositionPtr = multiword_items -> SLL[wordIndex];
        word_items -> match_mask       = multiword_items -> match_maskL[wordIndex];
        ...
        for (hitIndex2 = 0; hitIndex2 < twiceNumHits; hitIndex2 += 2)
        {
            ...
            {
                hitArray1[nextPosInHitArray + hitIndex1]     = hitArray[hitIndex2];
                hitArray1[nextPosInHitArray + hitIndex1 + 1] = start + hitArray1[nextPosInHitArray + hitIndex1 + 1];
            }
...
SPHIQueryInfo * SPHIQueryInfoNew()
{
    ...
    const Int4      kMinPhiLookupSize = 8;    /*
                                               *  Minimal allocation size for the array
                                               *   of pattern occurrences.
                                               */

    if ((pattern_info = (SPHIQueryInfo *) calloc(1, sizeof(SPHIQueryInfo))) == NULL)
    {
        return NULL;
    }
    ...
    if ((pattern_info -> occurrences = (SPHIPatternInfo *) calloc(kMinPhiLookupSize, sizeof(SPHIPatternInfo))) == NULL)
    {
        return NULL;
    }
...
SPHIQueryInfo * SPHIQueryInfoCopy(const SPHIQueryInfo * pat_info)
{
    ...
    retval                = (SPHIQueryInfo *) BlastMemDup(pat_info, sizeof(SPHIQueryInfo));
    retval -> pattern     = (char *) BlastMemDup(pat_info -> pattern, 1 + strlen(pat_info -> pattern));
    retval -> occurrences = (SPHIPatternInfo *) BlastMemDup(pat_info -> occurrences,
        ...
        Int4                                        length)
{
    ...
    {
        if ((occurrence_array = (SPHIPatternInfo *) realloc(pattern_info -> occurrences,
                2 * pattern_info -> allocated_size * sizeof(SPHIPatternInfo))) == NULL)
        {
            return -1;
        }
                              ...
                              BlastQueryInfo *             query_info)
{
    ...
    for (loc = location; loc; loc = loc -> next)
    {
        ...
        twiceNumHits = FindPatternHits(hitArray, sequence, loc_length, is_dna, pattern_blk);
        ...
        for (i = 0; i < twiceNumHits; i += 2)
        {
            ...
            s_PHIBlastAddPatternHit(pattern_info, hitArray[i + 1] + from, hitArray[i] - hitArray[i + 1] + 1);
        }
    }
