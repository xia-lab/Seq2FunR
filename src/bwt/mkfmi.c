         ...
         char ** argv)
{
    ...
    if (!fp)
    {
        error("File %s containing BWT could not be opened for reading\n", filename);
    }
    ...
    fprintf(stderr, "DONE\n");
    fprintf(stderr,
            "BWT of length %ld has been read with %d sequencs, alphabet=%s\n",
            b -> len,
            b -> nseq,
            b -> alphabet);
    ...
    if (!fp)
    {
        error("File %s containing SA could not be opened for reading\n", filename);
    }
    ...
    if (!fp)
    {
        error("File %s for FMI could not be opened for reading\n", filename);
    }
