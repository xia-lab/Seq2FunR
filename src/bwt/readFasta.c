...
static inline void test_seq_alloc(char * seq,
                                  char * seqend)
{
    if (seq >= seqend)
    {
        ERROR("Not enough memory allocated for sequence", 1);
    }
}
 ...
 */
static SEQstruct * read_id(FILE * fp,
                           char * idline)
{
    SEQstruct * ret;
    int         c = 1;
 ...
 *
 */
static SEQstruct * read_fasta_forward(FILE * fp,
        char *                               seq,
        long                                 length,
        char *                               translate,
        char                                 term,
...
SEQstruct * revcompSEQstruct(SEQstruct * ss,
                             char *      s,
                             char *      translate)
{
...
static void revcomp_all(SEQstruct * fbase,
                        char *      translate,
                        char        term,
                        int         padding)
{
    ...
    while (ss -> next)
    {
        ...
        if (!rbase)
        {
            rr         = rbase = revcompSEQstruct(ss, s, translate);
        }
        else
        {
            rr -> next = revcompSEQstruct(ss, s, translate);
            rr         = rr -> next;
        }
                      ...
                      int    padding)
{
    ...
    if (!ss)
    {
        ERROR("readFasta: No sequences read", 5);
    }
    ...
    if (complement)
    {
        if (totlen < 2 * ss -> len)
        {
            ERROR("readFasta: Not enough space allocated for reverse complement", 2);
        }
        ...
        for (i = 0; i < al; ++i)
        {
            revtrans[i] = transtab[(int) complement[i]];
        }
