 ...
 */
static inline int fmi_end_length(IndexType k,
                                 IndexType bwtlen)
{
 ...
 */
static inline IndexType fmi_chpt_value_with_dir(const FMI * f,
        const IndexType                                     k,
        const uchar                                         c,
        const int                                           direction)
{
 ...
 *  + some additionals
 */
static FMI * alloc_FMI_common(uchar *   bwt,
                              IndexType bwtlen,
                              int       alen,
                              size_t    index2_size)
{
    ...
    for (i = 0; i < f -> N1; ++i)
    {
        f -> index1[i] = (IndexType *) malloc(alen * sizeof(IndexType));
    }
    ...
    for (i = 0; i < f -> N2; ++i)
    {
        f -> index2[i] = (ushort *) malloc(index2_size * alen);
    }
 ...
 */
static FMI * makeIndex_common(uchar * bwt,
                              long    bwtlen,
                              int     alen)
{
    ...
    {
        if ((ii + 1) % ten_percent == 0)
        {
            fprintf(stderr, "%d%% ... ", 10 * (int) ((ii + 1) / ten_percent));
        }
        ...
        if ((ii > 0) &&!(ii & check2))
        {
            ...
            for (a = 0; a < alen; ++a)
            {
                fmi -> index2[R2][a] = (ushort) (total[a] - fmi -> index1[R1][a]);
            }
    ...
    for (a = 1; a < alen; ++a)
    {
        fmi -> index1[fmi -> N1 - 1][a] = fmi -> index1[fmi -> N1 - 1][a - 1] + total[a - 1];
    }

    for (R1 = 0; R1 < fmi -> N1 - 1; ++R1)
    {
        for (a = 1; a < alen; ++a)
        {
            fmi -> index1[R1][a] += fmi -> index1[fmi -> N1 - 1][a];
        }
    }
...
static void write_fmi_common(const FMI * f,
                             int         index2_size,
                             FILE *      fp)
{
    int i;
    ...
    for (i = 0; i < f -> N1; ++i)
    {
        fwrite(f -> index1[i], sizeof(IndexType), f -> alen, fp);
    }

    for (i = 0; i < f -> N2; ++i)
    {
        fwrite(f -> index2[i], 1, f -> alen * index2_size, fp);
    }
}
 ...
 */
static FMI * read_fmi_common(int    index2_size,
                             FILE * fp)
{
    int   i;
    FMI * f = (FMI *) malloc(sizeof(FMI));
