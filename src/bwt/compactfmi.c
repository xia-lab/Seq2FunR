...
static inline uchar fmi_decode_letter(uchar code)
{
    return lcode[code];
}

static inline uchar fmi_decode_number(uchar code)
{
    return ncode[code];
}

static void fmi_fill_codes(int   alen,
                           int * startLcode)
{
    int a, n, k;
...
static inline uchar encode_letter_number(uchar c,
        int                                    n,
        int *                                  startLcode)
{
    int max;
 ...
 */
static int * find_startLcode(const int       alen,
                             const uchar *   bwt,
                             const IndexType blen)
{
    IndexType i,
              tot        = 0,
              *count     = (IndexType *) calloc(alen, sizeof(IndexType));
    ...
    while (sum > 256)
    {
        ...
        for (a = 1; a < alen; ++a)
        {
            ...
            if ((maxNcode[a] > 2) && (maxNcode[a] < maxNcode[min]))
            {
                min = a;
            }
        }
    ...
    for (a = 0; a < alen; ++a)
    {
        startLcode[a + 1] = startLcode[a] + maxNcode[a];
    }
 ...
 */
static inline int fmi_bwt2number(const uchar c,
                                 uchar *     bwt,
                                 const int   direction)
{
    int n,
 ...
 */
static inline int fmi_chpt_difference(FMI * f,
        IndexType                           k,
        uchar                               c,
        int                                 direction)
{
    ...
    if (direction > 0)
    {
        ...
        if (chpt1 != chpt1up)
        {
            diff += (int) (f -> index1[c][chpt1up] - f -> index1[c][chpt1]);
        }
    }
 ...
 */
static inline uchar * find_closest_letter_with_bound(const uchar ct,
        uchar *                                                  bwt,
        const int                                                dir,
        const uchar *                                            bound)
{
                  ...
                  IndexType k)
{
    ...
    if (c != ct)
    {
        ...
        else
        {
            ...
            {
                bwt = find_closest_letter_with_bound(ct, bwt + direction, direction, bwtstop);
            }
        }
    }
    ...
    if (bwt)
    {
        fmi += delta + fmi_bwt2number(ct, bwt, direction);
    }
...
static IndexType FMindexHere(const FMI *     f,
                             uchar *         bwt,
                             const uchar     c,
                             const IndexType k)
{
...
/* Return the FMI value for the BWT letter at position k */
IndexType FMindexCurrent(FMI *     f,
                         uchar *   c,
                         IndexType k)
{
    uchar * bwt;
                ...
                IndexType * fmia)
{
    ...
    for (i = 0; i < f -> alen; ++i)
    {
        fmia[i] = size2;    // If letter has not been found, count = size2
    }
    ...
    {
        nleft = size2 - nleft;    // boundary for the search when direction = +1
    }

    if ((direction > 0) && (nleft > f -> bwtlen - k))
    {
        nleft = f -> bwtlen - k;
    }
    ...
    while (nleft > 0)
    {
        ...
        if (fmia[c] >= size2)
        {
            ...
            {
                ++fmia[c];    // Counting the number of times the letter returns 255
            }
        }
...
void FMIrecode(FMI * fmi)
{
    ...
    for (ii = 0; ii < fmi -> bwtlen; ++ii)
    {
        ...
        if ((ii > 0) &&!(ii & check2))
        {
            ...
            for (j = 0; j < size2 >> 1; ++j)
            {
                sbwt[j] = encode_letter_number(sbwt[j], deltaFmi[j], fmi -> startLcode);
            }

            for (; j < size2; ++j)
            {
                sbwt[j] = encode_letter_number(sbwt[j], (current[sbwt[j]] - deltaFmi[j]) - 1, fmi -> startLcode);
            }

            // for (   ; j<size2;   ++j)      sbwt[j] = encode_letter_number(sbwt[j],(current[sbwt[j]] - deltaFmi[j]),fmi->startLcode);
            for (a = 0; a < fmi -> alen; ++a)
            {
                current[a] = 0;
            }
    ...
    for (j = 0; (j < (size2 >> 1)) && (j < i); ++j)
    {
        sbwt[j] = encode_letter_number(sbwt[j], (uchar) deltaFmi[j], fmi -> startLcode);
    }

    for (; (j < size2) && (j < i); ++j)
    {
        sbwt[j] = encode_letter_number(sbwt[j], (uchar) (current[sbwt[j]] - deltaFmi[j]) - 1, fmi -> startLcode);
    }
