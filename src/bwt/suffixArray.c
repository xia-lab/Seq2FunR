...
static inline void long2uchar(long    k,
                              uchar * c,
                              int     n)
{
    while (n-- > 0)
 ...
 */
static inline void suffixArray_encode_number(int nseq,
        long                                     pos,
        uchar *                                  code,
        suffixArray *                            s)
{
 ...
 */
void suffixArray_make_hash(SEQstruct *   base,
                           suffixArray * s,
                           int           Hstep)
{
    SEQstruct * ss, **spt;
 ...
 */
static SEQstruct * hash_lookupSeq(char *        suffix,
                                  suffixArray * s)
{
    int i;
 ...
 */
suffixArray * init_suffixArray(SEQstruct * ss,
                               int         chpt_exp)
{
 ...
 */
void write_suffixArray_checkpoints(char **       sa,
                                   IndexType     start,
                                   IndexType     length,
                                   suffixArray * s,
                                   FILE *        sa_file)
...
/* Write SA header */
void write_suffixArray_header(suffixArray * s,
                              FILE *        fp)
{
    int i;
...
/* Read SA */
void read_suffixArray_body(suffixArray * s,
                           FILE *        fp)
{
    s -> sa = (uchar *) malloc(s -> ncheck * s -> nbytes * sizeof(uchar));
...
void write_suffixArray(suffixArray * s,
                       FILE *        fp)
{
