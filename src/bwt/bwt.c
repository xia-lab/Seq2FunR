...
static SI * alloc_SI(IndexType * si,
                     int         query_pos,
                     int         query_len)
{
 ...
 */
static inline int free_until_max_SI(SI * si,
        int                              max)
{
    int  n;
    SI * cur;
    ...
    while (cur -> next && (n - cur -> next -> count < max))
    {
        cur = cur -> next;
    }
...
static SI * insert_SI_sorted(SI * base,
                             SI * new)
{
    SI * tmp;
