...
class Stats
{
    public:
        ...
        void reportHtmlQuality(ofstream & ofs,
                               string     filteringType,
                               string     readName);

        void reportHtmlContents(ofstream & ofs,
                                string     filteringType,
                                string     readName);

        void reportHtmlKMER(ofstream & ofs,
                            string     filteringType,
                            string     readName);

        void reportHtmlORA(ofstream & ofs,
                           string     filteringType,
                           string     readName);
    ...
    public:
        static string list2string(double * list,
                                  int      size);
        ...
        static string list2string(long * list,
                                  int    size);

        static string list2string(std::vector<std::string> & x_vec,
                                  int                        top);

        static string list2string(std::vector<double> & y_vec,
                                  int                   top);

        static string list2string(std::vector<int> & x_vec,
                                  int                top);
