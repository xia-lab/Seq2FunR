...
{
    public:
        FilterResult(Options * opt,
                     bool      paired = false);

        ~FilterResult();

        inline long * getFilterReadStats()
        {
        ...
        // for paired end
        void addAdapterTrimmed(string adapter1,
                               string adapter2);

        void addPolyXTrimmed(int base,
                             int length);
        ...
        void reportAdapterJson(ofstream & ofs,
                               string     padding);

        // a part of JSON report for polyX trim
        void reportPolyXTrimJson(ofstream & ofs,
                                 string     padding);
        ...
        void addCorrection(char from,
                           char to);

        long getCorrectionNum(char from,
                              char to);
