...
FilterResult::FilterResult(Options * opt,
                           bool      paired)
{
    mOptions             = opt;
    mPaired              = paired;
...
void FilterResult::addCorrection(char from,
                                 char to)
{
    int f = from & 0x07;
    int t = to & 0x07;
...
long FilterResult::getCorrectionNum(char from,
        char                             to)
{
    int f = from & 0x07;
    int t = to & 0x07;
...
void FilterResult::addAdapterTrimmed(string adapter1,
        string                              adapter2)
{
    // paired
...
void FilterResult::addPolyXTrimmed(int base,
                                   int length)
{
...
void FilterResult::reportJson(ofstream & ofs,
                              string     padding)
{
    ofs << "{" << endl;
...
void FilterResult::outputAdaptersJson(ofstream & ofs,
...
void FilterResult::reportAdapterJson(ofstream & ofs,
        string                                  padding)
{
    ofs << "{" << endl;
...
void writeBaseCountsJson(ofstream & ofs,
                         string     pad,
                         string     key,
                         long       total,
                         long       (&counts)[4])
...
void FilterResult::reportPolyXTrimJson(ofstream & ofs,
        string                                    padding)
{
    ofs << padding << "{" << endl;
...
void FilterResult::outputAdaptersHtml(ofstream & ofs,
        map<string, long, classcomp> &           adapterCounts,
        long                                     totalBases)
{
    map<string, long>::iterator iter;
