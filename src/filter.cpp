...
bool Filter::filterByIndex(Read * r1,
                           Read * r2)
{
    if (mOptions -> indexFilter.enabled)
    {
...
bool Filter::match(vector<string> & list,
                   string           target,
                   int              threshold)
{
...
bool Filter::test()
{
    ...
    return (ret -> mSeq.mStr == "CCCCCCCCCCCCCCCCCCCCCCCCCCCC") && (ret -> mQuality == "CCCCCCCCCCC////CCCCCCCCCCCCC");
}
