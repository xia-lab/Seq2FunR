...
void Options::readDB() {
    ...
    if(!mHomoSearchOptions.genemap.empty()) {
        if (verbose) {
            std::string msg = "Reading gene species map from file " + mHomoSearchOptions.genemap;
            loginfo(msg);
        }
        std::vector<std::string> PVec;
		PVec.reserve(100000);
        mHomoSearchOptions.filein.open(mHomoSearchOptions.genemap.c_str());
		mHomoSearchOptions.dbUMap.clear();
		...
		while(mHomoSearchOptions.filein.getline(line, maxLine)){
			...
			splitStr(linestr, splittedVec, "\t");
			if (splittedVec.size() == 7){
				TaxonKO taxko;
				auto gnm = splittedVec[0];
				taxko.phylum = splittedVec[1];
				taxko.classm = splittedVec[2];
				taxko.order = splittedVec[3];
				taxko.family = splittedVec[4];
				taxko.genus = splittedVec[5];
				taxko.species = splittedVec[6];
				mHomoSearchOptions.dbUMap[gnm] = taxko;
			} else {
				error_exit(mHomoSearchOptions.genemap + " must consist 7 (for microbiome) columns.");
			}
		}
